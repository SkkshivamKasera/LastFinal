...
export const getProduct = (keyword="", currentPage=1, price=[0, 5000000], category, ratings=0) => async (dispatch) => {
    let link1 = `http://localhost:5000/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}`;
  if (category) {
    link1 = `http://localhost:5000/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
  }
    try{
...
export const getAllProductForAdmin = () => async(dispatch) => {
    try{
        ...
        const { data } = await axios.get("http://localhost:5000/api/v1/admin/products", {withCredentials: true})
...
export const getProductDetails = (id) => async (dispatch) => {
    try{
        let link2 = `http://localhost:5000/api/v1/product/${id}`
...
export const newProduct = (productData) => async (dispatch) => {
    try{
        ...
        const { data } = await axios.post('http://localhost:5000/api/v1/admin/products/new', productData, config)
...
export const deleteProduct = (id) => async (dispatch) => {
    try{
        ...
        const { data } = await axios.delete(`http://localhost:5000/api/v1/admin/products/${id}`, config)
...
export const updateProduct = (id, productData) => async (dispatch) => {
    try{
        ...
        const { data } = await axios.put(`http://localhost:5000/api/v1/admin/products/${id}`, productData, config)
